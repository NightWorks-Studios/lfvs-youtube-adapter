...
var YoutubeKeyManager = class {
  ...
  async init() {
    ...
    this.ctx.logger.info(`YoutubeKeyManager初始化完成。加载了 ${this.keys.length} 个Key，其中 ${this.exhaustedKeys.size} 个已耗尽。`);
  }
  async loadKeysFromFile() {
    this.keys = [];
    if (!fs.existsSync(this.keysFilePath)) {
      this.ctx.logger.warn(`未找到Key文件: ${this.keysFilePath}。将仅使用爬虫模式。`);
      return;
    }
  ...
  async loadStatus() {
    ...
    try {
      ...
      this.ctx.logger.error("读取Key状态文件失败，将重置状态:", error);
  ...
  async saveStatus() {
    ...
    try {
      ...
      this.ctx.logger.error("保存Key状态失败:", error);
    }
  }
  getPacificDateStr() {
  ...
  checkAndResetQuota() {
    ...
    if (currentPTDate !== this.lastResetDate) {
      if (this.exhaustedKeys.size > 0) {
        this.ctx.logger.info(`检测到日期变更 (PT: ${this.lastResetDate} -> ${currentPTDate})，重置所有Key状态。`);
        this.exhaustedKeys.clear();
      }
  ...
  async markKeyExhausted(key) {
    ...
    this.ctx.logger.warn(`Key额度耗尽或失效: ${key.substring(0, 8)}...`);
    ...
    if (apiKey) {
      try {
        ...
        if (error.response?.status === 403 && this.isQuotaError(error.response.data)) {
          ...
          return this.getVideoInfoAndStats(videoId);
        }
